...
Using DPoP means, client sending DPoP proof when requesting a token, plus client sendig DPoP proof along with access token to access the protected resources. DPoP proofs used in both steps by client are generated by client using a slightly different method.

So, DPoP proof can be generated by client, and it is a JWT. (this JWT is created by HTTP request data (API that client want to access), timestamp, a unique id and optionally a server nonce, plus access toen hash if request is having the token. And sign all these data elements using private key to create a JWT.

when requesting access token by sending token request to token endpoint, the client adds a `DPoP`
